...
Expected: not "<svg width=\"800\" height=\"200\" style=\"display: inline-block;\"><line x1=\"0\" y1=\"40\" x2=\"800\" y2=\"40\" stroke=\"#ccc\" stroke-width=\"1\"></line><line x1=\"0\" y1=\"55\" x2=\"800\" y2=\"55\" stroke=\"#ccc\" stroke-width=\"1\"></line><line x1=\"0\" y1=\"70\" x2=\"800\" y2=\"70\" stroke=\"#ccc\" stroke-width=\"1\"></line><line x1=\"0\" y1=\"85\" x2=\"800\" y2=\"85\" stroke=\"#ccc\" stroke-width=\"1\"></line><line x1=\"0\" y1=\"100\" x2=\"800\" y2=\"100\" stroke=\"#ccc\" stroke-width=\"1\"></line><text x=\"20\" y=\"30\" font-size=\"12\">ABC Music Notation</text></svg>"
```
...
- generic [ref=e2]:
  ...
  - generic [ref=e6]:
    - generic [ref=e7]:
      - generic [ref=e8]:
        - generic [ref=e9]: "Speed: 1.0x"
        - 'slider "Speed: 1.0x" [ref=e10] [cursor=pointer]': "1.5"
      - generic [ref=e11]:
        - generic [ref=e12]: "Piano Volume: 80"
        - 'slider "Piano Volume: 80" [ref=e13] [cursor=pointer]': "80"
      - generic [ref=e14]:
        - generic [ref=e15]: "Metronome Volume: 50"
        - 'slider "Metronome Volume: 50" [ref=e16] [cursor=pointer]': "50"
    - generic [ref=e17]:
      - button "▶ Play" [ref=e18] [cursor=pointer]
      - button "⏸ Pause" [disabled] [ref=e19] [cursor=pointer]
      - button "⏹ Stop" [disabled] [ref=e20] [cursor=pointer]
      - generic [ref=e21]: Ready
    - generic [ref=e22]:
      - button "Hide Input" [ref=e23] [cursor=pointer]
      - textbox "Enter ABC notation here..." [active] [ref=e24]: "K: G G A B"
    - img [ref=e27]:
      - generic [ref=e28]: ABC Music Notation
```
...
> 58  |       expect(newContent).not.toBe(initialContent);
  ...
  110 |       
  111 |       // Click again to verify it toggles back
  112 |       await toggleButton.click();
  113 |       await page.waitForTimeout(200);
  114 |       
  115 |       const afterSecondClick = await inputContainer.evaluate(el => 
  116 |         window.getComputedStyle(el).display
  117 |       );
  118 |       expect(afterSecondClick).toBe(initialDisplay);
  119 |     });
  120 | 
  121 |     test('should update button text when toggling visibility', async ({ page }) => {
  122 |       const toggleButton = page.locator('#abc-toggle-input');
  123 |       
  124 |       const initialText = await toggleButton.textContent();
  125 |       expect(initialText).toBeTruthy();
  126 |       
  127 |       // Click to hide
  128 |       await toggleButton.click();
  129 |       await page.waitForTimeout(200);
  130 |       
  131 |       const afterClick = await toggleButton.textContent();
  132 |       expect(afterClick).not.toBe(initialText);
  133 |       
  134 |       // Click to show
  135 |       await toggleButton.click();
  136 |       await page.waitForTimeout(200);
  137 |       
  138 |       const afterSecondClick = await toggleButton.textContent();
  139 |       expect(afterSecondClick).toBe(initialText);
  140 |     });
  141 |   });
  142 | 
  143 |   test.describe('URL State Management', () => {
  144 |     test('should preserve textarea content as URL state', async ({ page }) => {
  145 |       const textarea = page.locator('#abc-text-area');
  146 |       
  147 |       // Enter new content
  148 |       const newAbc = 'K: D\nD E F G';
  149 |       await textarea.clear();
  150 |       await textarea.fill(newAbc);
  151 |       
  152 |       // Wait for debounce
  153 |       await page.waitForTimeout(2500);
  154 |       
  155 |       // Check URL contains encoded state
  156 |       const url = page.url();
  157 |       expect(url).toContain('state=');
  158 |     });
```